            ...
            <NavigationMenu>
              <NavigationMenuList>
                ...
                <NavigationMenuItem>
                  ...
                  <NavigationMenuContent>
                    <div className="grid grid-cols-2 gap-2 p-4 w-[400px]">
                      {indiaDestinations.map(dest => <Link key={dest} to={`/destinations/${dest.toLowerCase().replace(/\s+/g, "-")}`} className="block p-2 rounded-lg hover:bg-muted transition-colors">
                          {dest}
                        </Link>)}
                    </div>
                  </NavigationMenuContent>
                ...
                <NavigationMenuItem>
                  ...
                  <NavigationMenuContent>
                    <div className="grid grid-cols-2 gap-2 p-4 w-[400px]">
                      {globalDestinations.map(dest => <Link key={dest} to={`/destinations/${dest.toLowerCase().replace(/\s+/g, "-")}`} className="block p-2 rounded-lg hover:bg-muted transition-colors">
                          {dest}
                        </Link>)}
                    </div>
                  </NavigationMenuContent>
                ...
                <NavigationMenuItem>
                  ...
                  <NavigationMenuContent>
                    <div className="grid gap-2 p-4 w-[300px]">
                      {otherServices.map(service => <Link key={service} to={`/services/${service.toLowerCase().replace(/\s+/g, "-")}`} className="block p-2 rounded-lg hover:bg-muted transition-colors">
                          {service}
                        </Link>)}
                    </div>
                  </NavigationMenuContent>
            ...
            <div className="relative ml-4">
              ...
              <AnimatePresence>
                ...
                width: 0
                    ...
                    <input type="text" placeholder="Search packages..." className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary" />
                  </motion.div>}
              </AnimatePresence>
            </div>
          </div>
              ...
              <div className="container-custom py-4 space-y-4">
                ...
                <details className="group">
                  ...
                  <div className="pl-4 space-y-2 mt-2">
                    {indiaDestinations.map(dest => <Link key={dest} to={`/destinations/${dest.toLowerCase().replace(/\s+/g, "-")}`} className="block py-1 text-muted-foreground hover:text-primary" onClick={() => setIsMenuOpen(false)}>
                        {dest}
                      </Link>)}
                  </div>
                </details>
                <details className="group">
                  ...
                  <div className="pl-4 space-y-2 mt-2">
                    {globalDestinations.map(dest => <Link key={dest} to={`/destinations/${dest.toLowerCase().replace(/\s+/g, "-")}`} className="block py-1 text-muted-foreground hover:text-primary" onClick={() => setIsMenuOpen(false)}>
                        {dest}
                      </Link>)}
                  </div>
                </details>
